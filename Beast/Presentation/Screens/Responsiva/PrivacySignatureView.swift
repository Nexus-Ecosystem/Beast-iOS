...
struct PrivacySignatureView: View {
    ...
    @StateObject private var viewModel = PrivacySignatureViewModel()
    @State private var canvasView = PKCanvasView()
    @State private var isSigning = false
    @State private var showEmptySignatureError = false

    var onSignatureComplete: () -> Void = {}
        ...
        ZStack {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 24) {
                    ...
                    Text("AL PRESIONAR \"FIRMAR\", CONFIRMAS QUE HAS LEÍDO Y ACEPTAS LOS TÉRMINOS ANTERIORES.")
            ...
            if viewModel.isLoading {
                BeastLoadingOverlay(message: "Firmando responsiva...")
                    .zIndex(100)
            }
            ...
            if showEmptySignatureError {
                BeastAlertDialog(
                    style: .error,
                    title: "Firma requerida",
                    message: "Realiza tu firma antes de continuar.",
                    buttonTitle: "Entendido"
                ) {
                    showEmptySignatureError = false
                }
                .zIndex(400)
            }
        }
        .navigationTitle("Responsiva")
    ...
    private var legalCard: some View {
        VStack(alignment: .leading, spacing: 24) {
            LegalSectionView(
                number: "1",
                title: "CONDICIÓN FÍSICA Y SALUD DEL USUARIO",
                content: "Al inscribirte y utilizar nuestras instalaciones, declaras bajo protesta de decir verdad que te encuentras en óptimas condiciones físicas y de salud para realizar actividades de alta intensidad. Es tu responsabilidad exclusiva informar al instructor sobre cualquier padecimiento, lesión, embarazo o condición médica preexistente antes de iniciar la sesión."
            )

            LegalSectionView(
                number: "2",
                title: "USO RESPONSABLE DE EQUIPOS Y BICICLETAS",
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .stroke(BeastColors.border, lineWidth: 1)
        )
    }
    ...
    private func sign() {
        ...
        Task {
            await viewModel.sign(image: image)
        }
    }
}
